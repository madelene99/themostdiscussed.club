...
define('CAMPAIGN_ID', "b419500fe9fe2b932c6a2eb3dec240bc");
define('REQUEST_LIVE_TIME', 3600);
define('ENC_KEY', 'bae3a1ff6e97f682a2ef6cd4aafb86c9');
define('MP_PARAM_NAME', '__os__sess');
define('NOT_FOUND_TEXT', '<h1>Page not found</h1>');
define('CHECK_MCPROXY', 0);

function translateCurlError($code) {$output = '';$curl_errors = array(2  => "Can't init curl. (code: 2)",6  => "Can't resolve server's DNS of our domain. Please contact your hosting provider and tell them about this issue. (code: 6)",7  => "Can't connect to the server. (code: 7)",28 => "Operation timeout. Check you DNS setting. (code: 28)");if (isset($curl_errors[$code])) $output = $curl_errors[$code];else $output = "Error code: $code . Check if php cURL library installed and enabled on your server.";return $output;}
...
function sendRequest($data, $path = 'index') {
    $headers = array('adapi' => '2.1');
    $data_to_post = array("cmp" =>  CAMPAIGN_ID,"headers" => $data,"adapi" => '2.1');    
    
    $ch = curl_init("http://check.magicchecker.com/v2.1/" .$path .'.php');
...
if (CHECK_MCPROXY) {if (isset($_SERVER['HTTP_MC_CACHE']) && $_SERVER['HTTP_MC_CACHE']) {print 'mcproxy is ok';} else {print 'mcproxy error';}die();}
if (isset($_SERVER['HTTP_MC_IP']) && $_SERVER['HTTP_MC_IP']) {$_SERVER['REMOTE_ADDR'] = $_SERVER['HTTP_MC_IP']; unset($_SERVER['HTTP_MC_IP']);}
